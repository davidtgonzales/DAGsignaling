...
################################################
# DAG profile likelihoods on experimental data #
################################################
# load data
...
pl_arr, pj_arr, pr_arr, pf_arr = get_pl_DAG(fdx,r_params[end,fdx],cells,t_data[:,1:cells],c1_data[:,1:cells],0,r_max,c1_recruit[1:cells],dosetimes,p_init)

# save results
save(string("./out/experiment/pl_DAG_",set_id,"_",string(fdx),".jld"), "pl_arr", pl_arr)
save(string("./out/experiment/pj_DAG_",set_id,"_",string(fdx),".jld"), "pj_arr", pj_arr)
save(string("./out/experiment/pr_DAG_",set_id,"_",string(fdx),".jld"), "pr_arr", pr_arr)
save(string("./out/experiment/pf_DAG_",set_id,"_",string(fdx),".jld"), "pf_arr", pf_arr)
